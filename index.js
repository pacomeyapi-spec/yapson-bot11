...
let state = {
  ...
  cookiesReady: false,
  cookies: null,
  yapsonToken: YAPSON_TOKEN, // ← injectable depuis le dashboard
};
...
function fmtAmt(n) { return n.toLocaleString('fr-FR'); }
...
async function yapsonFetchMessages(fromTs, toTs) {
  const token = state.yapsonToken || YAPSON_TOKEN; // state.yapsonToken en priorité
...
async function yapsonApprove(msgId) {
  const token = state.yapsonToken || YAPSON_TOKEN;
...
async function ensureLoggedIn() {
  await ensureBrowser();
  if (!state.cookiesReady || !state.cookies) { await mgmtLogin(); return; }
  try {
    const url = page.url();
    if (!url || url.includes('login') || url === 'about:blank') await mgmtLogin();
...
async function runF2() {
  ...
  log(`F2 — ${rows.length} ligne(s)`);
}

// ── F3 ────────────────────────────────────────────────────────
// Config F3 dynamique
let f3Config = { marginMin: F3_MARGIN_MIN, rejectMin: F3_REJECT_MIN };
...
async function runF3() {
  ...
  await page.goto(`${MGMT_URL}/fr/admin/report/pendingrequestrefill`, { waitUntil: 'networkidle', timeout: 30000 });
  try {
    const toggleInput = await page.$('input[type="checkbox"].toggle, .toggle input, input#autoUpdate');
    if (toggleInput) { const isOn = await toggleInput.isChecked(); if (isOn) await toggleInput.dispatchEvent('click'); }
    await page.waitForTimeout(500);
    const applyBtn = await page.$('button:has-text("APPLIQUER")');
    if (applyBtn) { await applyBtn.click(); await page.waitForTimeout(2000); }
  } catch(e) { log(`⚠ Setup: ${e.message.substring(0,80)}`); }
  ...
  const now = Date.now();
  let oldestTs = now, oldestStr = null;
  for (const row of pendingRows) {
    const d = parseMgmtDate(row.cells[0]);
    if (d && !isNaN(d.getTime()) && d.getTime() < oldestTs) { oldestTs = d.getTime(); oldestStr = row.cells[0]; }
  }
  ...
  let yapMessages;
  try { yapMessages = await yapsonFetchMessages(fromTs, toTs); }
  catch(e) { log(`F3 ❌ Erreur YapsonPress: ${e.message}`); return; }
  ...
  for (const msg of yapMessages) {
    ...
    if (!parsed) continue;
    payments.push({ phone: parsed.phone, amount: parsed.amount, msgId: msg.id || msg._id, approved: msg.status === 'approuve' || msg.status === 'approved', sender: msg.sender });
  }
  log(`F3 — ${payments.length} paiement(s) parsé(s)`);

  log('F3 [3/5] Approbation dans YapsonPress…');
  let approvedCount = 0;
  for (const p of payments) {
    if (!p.approved && p.msgId) { const ok = await yapsonApprove(p.msgId); if (ok) { approvedCount++; p.approved = true; } }
  }
  log(`F3 — ${approvedCount} approuvé(s) dans YapsonPress`);
  ...
  const yapMap = {};
  for (const p of payments) { if (!yapMap[p.phone]) yapMap[p.phone] = []; yapMap[p.phone].push(p); }
  ...
  for (let ri = 0; ri < rowCount; ri++) {
    ...
    try {
      ...
      if (matches && matches.length > 0) {
        ...
        const best = exactMatch || matches.reduce((a,b) => Math.abs(a.amount-(reqAmount||0)) <= Math.abs(b.amount-(reqAmount||0)) ? a : b);
        const montantCorrigé = best.amount;
        if (reqAmount && reqAmount !== montantCorrigé) log(`F3 — Correction ${reqPhone}: ${fmtAmt(reqAmount)}→${fmtAmt(montantCorrigé)}F`);
        try {
          let confirmLink = null;
          for (const a of await rowHandle.locator('a').all()) { if ((await a.textContent()).trim() === 'Confirmer') { confirmLink = a; break; } }
      ...
      } else if (ageMin >= rejectMin) {
        log(`F3 — Rejet: ${reqPhone} introuvable, âge ${ageMin.toFixed(0)} min >= ${rejectMin} min`);
        try {
          let rejectLink = null;
          for (const a of await rowHandle.locator('a').all()) { if ((await a.textContent()).trim() === 'Rejeter') { rejectLink = a; break; } }
          if (!rejectLink) { log(`F3 ⚠ Lien Rejeter non trouvé`); continue; }
...
function setPaused(val) {
  paused = val;
  if (paused) { state.status = 'paused'; log("⏸ Bot mis en pause"); }
  else { state.status = state.cookiesReady ? 'running' : 'waiting_cookies'; log("▶ Bot relancé"); }
}

// ── Boucle principale ──────────────────────────────────────────
async function mainLoop() {
  log(`🤖 Bot démarré — Fonction: ${FONCTION} | Intervalle: ${INTERVAL_SEC}s`);
  if (FONCTION === 'F3') log(`⚙ F3 config: marge=${f3Config.marginMin}min | seuil rejet=${f3Config.rejectMin}min`);
  state.status = state.cookies ? 'running' : 'waiting_cookies';
  while (true) {
    if (paused) { await new Promise(r => setTimeout(r, 2000)); continue; }
    try {
      state.polls++;
      state.lastRun = new Date().toISOString();
      if (FONCTION === 'F1') await runF1();
      else if (FONCTION === 'F2') await runF2();
      else if (FONCTION === 'F3') await runF3();
      else log(`❌ FONCTION inconnue: ${FONCTION}`);
    } catch(e) {
      state.errors++;
      log(`❌ Erreur cycle: ${e.message}`);
      if (!paused) state.status = 'error';
      if (e.message.includes('ookies')) log('🍪 En attente de nouveaux cookies…');
...
app.get('/', (req, res) => {
  ...
  const controlButtons = `
    <div style="margin-bottom:16px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      ...
      <form method="POST" action="/control" style="display:inline">
        ...
        <button class="btn-neutral" type="submit">🍪 Réinitialiser cookies</button>
      </form>
  ...
  const yapsonTokenForm = `
    <form method="POST" action="/inject-yapson-token"
      style="background:#1e1e2e;border-radius:10px;padding:16px;margin-bottom:16px">
      <div style="color:#89b4fa;font-size:13px;font-weight:bold;margin-bottom:8px">
        🔑 Token YapsonPress ${tokenOk ? '✅ Actif' : '❌ Requis'}
      </div>
      <div style="font-size:11px;color:#6c7086;margin-bottom:10px">
        Colle le token Bearer de YapsonPress ci-dessous.<br>
        F12 sur YapsonPress → Console →
        <code style="background:#313244;padding:2px 5px">copy(localStorage.getItem('token'))</code>
      </div>
      <input type="password" name="yapsonToken"
        placeholder="eyJhbGci... ou token Bearer"
        value="${tokenOk ? '●'.repeat(24) : ''}"
        style="width:100%;background:#313244;color:#cdd6f4;border:1px solid #45475a;
               border-radius:6px;padding:8px;font-size:11px;font-family:monospace;margin-bottom:8px">
      <button type="submit"
        style="background:#89b4fa;color:#1e1e2e;border:none;border-radius:6px;
  ...
  const cookiesForm = !state.cookiesReady ? `
    <form method="POST" action="/cookies"
      style="background:#1e1e2e;border-radius:10px;padding:16px;margin-bottom:16px">
      <div style="color:#f38ba8;font-size:13px;font-weight:bold;margin-bottom:8px">
        🍪 Cookies my-managment requis
      </div>
      <div style="font-size:11px;color:#6c7086;margin-bottom:10px">
        1. Connecte-toi sur my-managment.com dans ton navigateur<br>
        2. F12 → Application → Cookies → my-managment.com<br>
        3. Copie tout en JSON et colle ci-dessous
      </div>
      <textarea name="cookies" rows="4"
        placeholder='[{"name":"session","value":"...","domain":".my-managment.com",...}]'
        style="width:100%;background:#313244;color:#cdd6f4;border:1px solid #45475a;
               border-radius:6px;padding:8px;font-size:11px;font-family:monospace;resize:vertical"></textarea>
      <button type="submit"
        style="margin-top:8px;background:#a6e3a1;color:#1e1e2e;border:none;border-radius:6px;
               padding:8px 16px;font-size:13px;font-weight:bold;cursor:pointer">
  ...
  const f3ConfigForm = (FONCTION === 'F3') ? `
    <form method="POST" action="/f3-config"
      style="background:#1e1e2e;border-radius:10px;padding:14px;margin-bottom:16px;
      ...
      <label style="font-size:12px;color:#cdd6f4">
        ...
        <select name="marginMin"
          style="background:#313244;color:#cdd6f4;border:1px solid #45475a;border-radius:6px;padding:6px 10px;font-size:13px">
      ...
      <button type="submit"
        style="background:#a6e3a1;color:#1e1e2e;border:none;border-radius:6px;padding:7px 14px;font-size:12px;font-weight:bold;cursor:pointer">
        Appliquer
      </button>
      <span style="font-size:11px;color:#6c7086">
        Actuel : marge=${f3Config.marginMin}min | rejet>=${f3Config.rejectMin}min
      </span>
    </form>` : '';

  const statusLabels = {
    running: '🟢 Actif', error: '🔴 Erreur', paused: '⏸ En pause',
    waiting_cookies: '🍪 Cookies requis', starting: '🔵 Démarrage',
  };
  const html = DASHBOARD_HTML
    ...
    .replace('{{YAPSON_TOKEN_FORM}}', yapsonTokenForm)
...
// ── Routes ─────────────────────────────────────────────────────
app.post('/inject-yapson-token', (req, res) => {
  const raw = (req.body.yapsonToken || '').trim();
  if (raw && !raw.startsWith('●')) {
    state.yapsonToken = raw;
    log(`🔑 Token YapsonPress injecté (${raw.length} chars)`);
  }
  res.redirect('/');
});
...
app.post('/control', (req, res) => {
  ...
  if (action === 'stop') setPaused(true);
  else if (action === 'start') setPaused(false);
  else if (action === 'reset_cookies') {
    state.cookies = null; state.cookiesReady = false; state.status = 'waiting_cookies';
...
app.post('/cookies', (req, res) => {
  ...
  } catch(e) { log(`❌ JSON cookies invalide: ${e.message}`); }
  res.redirect('/');
});
...
app.post('/f3-config', (req, res) => {
  ...
  if (allowed.includes(newMargin)) { f3Config.marginMin = newMargin; log(`⚙ F3 marge mise à jour : ${newMargin} min`); }
  if (newReject >= 10 && newReject <= 120) { f3Config.rejectMin = newReject; log(`⚙ F3 seuil rejet mis à jour : ${newReject} min`); }
  res.redirect('/');
});
